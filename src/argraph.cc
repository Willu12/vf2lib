...
template <class T> inline void grow(T*& ptr, int old_size, int new_size) {
...
template <class T> inline void clear(T* ptr, int n) {
    int i;
    for (i = 0; i < n; i++)
        ptr[i] = (T)0;
}
...
void ARGraph_impl::SetNodeAttr(node_id i, void* new_attr, bool destroyOld) {
    assert(i < n);
    if (destroyOld) DestroyNode(attr[i]);
    attr[i] = new_attr;
}
 ...
 * Note: uses binary search.
 ------------------------------------------------------------------*/
void ARGraph_impl::SetEdgeAttr(node_id n1, node_id n2, void* new_attr, bool destroyOld) {
    int a, b, c;
    node_id* id;
    ...
    while (a < b) {
        ...
        if (id[c] < n2)
            ...
            if (destroyOld) DestroyEdge(out_attr[n1][c]);
            out_attr[n1][c] = new_attr;
            break;
        }
    }
    if (a >= b) error("ARGraph_impl::SetEdgeAttr: non existent edge");
    ...
    while (a < b) {
        ...
        if (id[c] < n1)
            ...
            b = c;
        else { // The old attr here is intentionally
    ...
    if (a >= b) error("ARGraph_impl::SetEdgeAttr: inconsistent graph state");
}
 ...
 * Applies the visitor to all the 'in' edges of 'node'
 ------------------------------------------------------------------*/
void ARGraph_impl::VisitInEdges(node_id node, edge_visitor vis, param_type param) {
    assert(node < n);
    int i;
 ...
 * Applies the visitor to all the 'out' edges of 'node'
 ------------------------------------------------------------------*/
void ARGraph_impl::VisitOutEdges(node_id node, edge_visitor vis, param_type param) {
    assert(node < n);
    int i;
...
void ARGraph_impl::DestroyNode(void* attr) {
    if (node_destroyer != NULL) node_destroyer->destroy(attr);
}
...
void ARGraph_impl::DestroyEdge(void* attr) {
    if (edge_destroyer != NULL) edge_destroyer->destroy(attr);
}
